      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 lg:gap-[30px] md:gap-[80px] gap-[30px] mt-6 lg:scale-100 md:scale-[80%] scale-100 items-start justify-start lg:ml-0 md:ml-[-70px] ml-0 lg:mt-[80px] md:mt-[-100px]">
          {products.map((product) => (
            <div
              key={product.id}
              className="hover:scale-105 transition-all text-left lg:w-[238px] md:w-[238px] w-[328px] h-[715px] border-2 px-4 rounded-lg flex flex-col items-center justify-start drop-shadow-lg gap-4"
            >
              <Link to={`/product/${product.id}`}>
                <img
                  src={product.image}
                  alt={product.title}
                  className="w-[238px] h-[427px] object-contain mb-2 border-b-2 border-black"
                />
              </Link>
