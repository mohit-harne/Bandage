...
import shopimg4 from '../images/shopimg4.jpg'; 
import shopimg5 from '../images/shopimg5.jpg'; 
import shopimg6 from '../images/shopimg6.jpg'; 
...
const Slider2 = () => {
  ...
  return (
    <div className="relative w-full lg:h-[716px] md:h-[716px] h-[753px] overflow-hidden">
      ...
      >
        <img src={leftarrow} alt="Previous" className="lg:scale-100 md:scale-90 scale-[60%] h-[44px] w-[24px]" />
      </button>
      <button
        onClick={handleNext}
        className="absolute top-1/2 right-4 transform -translate-y-1/2  "
      >
        <img src={rightarrow} alt="Next" className="lg:scale-100 md:scale-90 scale-[60%] h-[44px] w-[24px]" />
      </button>
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-[2px] lg:scale-100 md:scale-100 scale-75">
        {images.map((_, index) => (
          <div
            key={index}
            className={`w-[62px] h-[10px]  ${index === currentIndex ? 'bg-white' : 'bg-gray-400'}`}
          />
        ))}
      </div>
    </div>
